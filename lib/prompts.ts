  ...
  return [
    {
      ...
      content:
        "你是一名给中文文章做插画配图策略的资深分镜师。你的工作是通读文章，找出值得配图的位置，输出结构化的 shot list。你必须遵守：1) 一篇 1000-1500 字的文章配 4-6 张图，不要平均铺，只打认知锚点；2) 跳过一句话转折段和纯情绪口号结尾——配图会掐断节奏；3) 结构类型限定：前后对比 / 概念隐喻 / 闭环循环 / 过程分解；4) 每张图只讲一个核心结构；5) 画面必须有一个明确的小怪角色承担核心概念动作，不能只是装饰。输出必须是合法 JSON，不要输出任何解释文字。"
    },
    {
      role: "user" as const,
...
要求输出 JSON：
{"shots":[{"position":"放在哪一段之后，如：第1段后 / 论点三段后","theme":"这一张的主题，一句话","core_idea":"核心意思，一到两句口语化中文","structure_type":"前后对比|概念隐喻|闭环循环|过程分解","xiaohei_action":"小黑（画面主角，一只实心黑色、白点眼睛、细腿、面无表情的小怪）在画面里执行的核心动作，写具体","labels":["建议印在图上的中文标注词，3-6个，每个不超过8字"],"elements":["建议出现的视觉元素，4-6个"]}]}

插画风格 DNA（生成画面时遵守）：${settings.styleDna}
...
export function buildImagePrompt(shot: Shot, settings: Settings) {
...
Chinese handwritten labels (use at most 5-8 of them):
${labels}

Constraints:
One image explains only one core structure. Keep the main subject around 40%-60% of the canvas. Preserve at least 35% blank white space. Use short handwritten Chinese labels only, no long sentences. Do not write a title in the top-left corner. Do not write the structure type on the image. Do not make it a formal diagram, course slide, or dense explainer. It should be clear but not instructional, interesting but not childish, strange but clean. No gradients, no shadows, no paper texture, no complex background.`;
}
